...
object NBTPackage {
     ...
     *
     * This method initializes specified classes by referencing their class names to ensure that
     * they are loaded into the JVM. This is commonly used to trigger static initializers or to
     * ensure the availability of certain classes at runtime.
     *
     * The classes being loaded pertain to various `Tag` implementations, which represent different
     * types of NBT elements and associated I/O utilities.
     */
    fun forceLoad() {
        val classes =
            arrayOf(
...
sealed class TagType<T : Tag>(val code: kotlin.Int, val clazz: KClass<T>, val name: kotlin.String) {
    ...
    companion object {
        ...
        fun fromCode(code: kotlin.Byte): TagType<out Tag> =
            codeToTagType[code.toInt()]
                ?: throw IllegalArgumentException("Invalid tag code ($code)")
        ...
        fun <T : Tag> fromClass(clazz: KClass<T>): TagType<T> =
            (kclazzToTagType[clazz] as? TagType<T>)
                ?: throw IllegalArgumentException("Invalid tag class ($clazz)")
    }
}

/**
 * A sealed class representing a generic NBT (Named Binary Tag) structure. This is used to serialize
 * and deserialize hierarchical data. Subclasses of `Tag` represent specific data types, such as
 * numbers, lists, strings, or compound structures.
 */
sealed class Tag {
     ...
     * @return `true` if the tag is a `ListTag` with elements of the specified type or if it matches
     *   the criteria for an empty `ListTag` with an `EndTag` element type; otherwise, `false`.
     */
     ...
     *
     * This function checks if the current tag is a `ListTag` containing elements of type `T` or
     * satisfies the criteria for an empty `ListTag` with an `EndTag` as the element type. If these
     * conditions are met, it returns the tag as a `ListTag<T>`. Otherwise, it returns null.
     *
     * @return A `ListTag<T>` if the current tag matches the conditions for being a list of type
     *   `T`, or null otherwise.
     */
    @OptIn(ExperimentalContracts::class)
     ...
     * @return A `ByteArray` containing the serialized representation of the tag with the given
     *   name.
     */
    ...
    /**
     * Appends the SNBT (String Named Binary Tag) representation of the current tag to the provided
     * `StringBuilder`.
     *
    ...
    companion object {
         ...
         *
         * @param data The byte array containing the raw serialized data to be converted into a
         *   `Tag`.
 ...
 *
 * A `NamedTag` is composed of a name and an associated `Tag` object. This is used for storing and
 * organizing hierarchical data within NBT structures, where each data entry is identified by a
 * unique name.
 *
 ...
 *
 * The EndTag signifies the termination of a compound or list tag in the NBT structure. It is
 * primarily used as a marker and does not contain any associated value, serving as a delimiter for
 * the hierarchical structure of NBT data.
 */
object EndTag : Tag() {
 ...
 *
 * This class is designed to encapsulate a list of `Tag` objects and validate that all elements in
 * the list conform to the specified type. It throws warnings for invalid or overly generic tag
 ...
 * @throws IllegalStateException when the list is heterogeneous or does not conform to the specified
 *   type
 */
...
/**
 * Represents a compound tag that is a collection of key-value pairs where the key is a string, and
 * the value is a tag. Provides various utility methods to interact with and retrieve values from
 * the compound tag.
 *
 * @property value A mutable map storing the key-value pairs, where the key is a string and the
 *   value is a tag.
...
data class CompoundTag(var value: MutableMap<String, Tag>) : Tag() {
     ...
     * @return `true` if a tag is associated with the given key in the compound tag, `false`
     *   otherwise.
     */
     ...
     * @return The `ByteArray` associated with the given key, or `null` if no `ByteArray` is found
     *   for the key.
     */
     ...
     * @return The `IntArray` associated with the given key, or `null` if no `IntArray` is found for
     *   the key.
     */
    ...
    /**
     * Retrieves a list of tags of the specified type associated with the given key from the
     * compound tag.
     *
     ...
     * @return The list of tags of the specified type associated with the given key, or `null` if no
     *   list of the matching type exists for the key.
     */
    fun <T : Tag> getList(key: String, clazz: Class<T>): List<T>? {
        val list = value[key] as? ListTag<*>
        @Suppress("UNCHECKED_CAST")
        return if (list?.elementType == clazz) {
...
/**
 * Creates a `CompoundTag` from a variable number of key-value pairs representing tags. This
 * function initializes and populates a `CompoundTag` with the given key-value pairs, where each key
 * is a string and each value is a `Tag`.
 *
 * @param values The key-value pairs to be added to the `CompoundTag`. Each pair consists of a
 *   string key and a `Tag` value.
 ...
 *
 * @param values A collection of pairs where the first element is a `String` representing the key,
 *   and the second element is a `Tag` representing the value to be included in the `CompoundTag`.
 * @return A `CompoundTag` containing the provided key-value pairs.
 */
